...
const renderer=new THREE.WebGLRenderer({alpha:true,antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<700?1:1.15));renderer.setSize(host.clientWidth,host.clientHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.3;host.prepend(renderer.domElement);
...
let sceneVisible=true;new IntersectionObserver(([entry])=>{sceneVisible=entry.isIntersecting},{rootMargin:'100px'}).observe(host);function animate(stamp=0){requestAnimationFrame(animate);if(document.hidden||!sceneVisible||stamp-last<42)return;last=stamp;const t=stamp*.001;globe.rotation.y+=.0028+(targetY*.0015);globe.rotation.x+=(targetX*.08-globe.rotation.x)*.03;core.position.y=2.75+Math.sin(t*1.5)*.12;core.rotation.y=t*.5;beacons.forEach((b,i)=>{b.children[1].scale.setScalar(1+Math.sin(t*3+i)*.25)});renderer.render(scene,camera)}animate();
